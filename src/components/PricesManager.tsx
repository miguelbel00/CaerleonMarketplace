...
function PricesManager() {
  ...
  const fetchFixedRunes = async () => {
    ...
    for (const element of runesFixed) {
      ...
      allAsyncResults.push(...asyncResult.map(e => ({ ...e, name: element.LocalizedNames[languaje] })))
    }
    return allAsyncResults
  }
  ...
  const retrieveData = async (actualUniqueName?: string, actualCity?: string) => {
    ...
    let itemName: string
    if (useful) {
      itemName = useful?.LocalizedNames ? useful.LocalizedNames[languaje] : (actualUniqueName || manualItem)
    } else if (equip) {
      itemName = equip?.LocalizedNames ? equip.LocalizedNames[languaje] : (actualUniqueName || manualItem)
    }
